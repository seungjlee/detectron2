...
BOX_HEAD_CHANNELS = 256  # For original model configuration, set this to 256.
BOX_HEAD_CONVOLUTIONS = 4  # For original model configuration, set this to 4.
BOX_HEAD_FULLY_CONNECTED_DIM = 1024  # For original model configuration, set this to 1024.

MAX_HEAD_CHANNELS = 256  # For original model configuration, set this to 256.
MAX_HEAD_CONVOLUTIONS = 4  # For original model configuration, set this to 4.
...
model.roi_heads.update(
    box_head=L(FastRCNNConvFCHead)(
        input_shape=ShapeSpec(channels=BOX_HEAD_CHANNELS, height=7, width=7),
        conv_dims=[BOX_HEAD_CHANNELS] * BOX_HEAD_CONVOLUTIONS,
    ...
    mask_head=L(MaskRCNNConvUpsampleHead)(
        input_shape=ShapeSpec(channels=MAX_HEAD_CHANNELS, width=14, height=14),
        num_classes="${..num_classes}",
        conv_dims=[MAX_HEAD_CHANNELS] * (MAX_HEAD_CONVOLUTIONS + 1),
        conv_norm="LN",
    ),
)
